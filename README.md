...
2. Run `node src/sync_severity_ui.js` (or `node src/sync_embed.js`) to update the import dashboard (**Severity only** — Resource Selector is left as the working previous copy).
...
See [Performance & filters](docs/improve_dashboard_performance_and_filters.md) for the Severity speedups (parallel pages, name chunks, progressive UI + loading banner).
