(function () {
   ...
   * - Never send invalid systemProperties/customProperties JSON blobs to /device/devices
   * - Resolve category/metadata filters client-side (same approach as FilterWidget)
  ...
  const META_PROPS = [
    { n: 'system.categories', multi: true },
    { n: 'customer', multi: false },
    { n: 'department', multi: false },
    { n: 'device_type', multi: false },
    { n: 'location', multi: false }
  ];
  ...
  /** Categories may be comma/space separated; other props are exact (case-insensitive). */
  function propertyMatches(rawValue, wantedValues, multiToken) {
    ...
    if (multiToken) {
      const tokens = raw.split(/[,;\s]+/).map(function (t) { return t.trim().toLowerCase(); }).filter(Boolean);
  ...
  function deviceMatchesMetaFilters(device, activeMeta) {
    for (let i = 0; i < META_PROPS.length; i++) {
      ...
      if (!propertyMatches(raw, vals, p.multi)) return false;
    }
    return true;
  }
  ...
  function readDeviceCache() {
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.ts && (Date.now() - parsed.ts) < DEVICE_CACHE_TTL_MS && Array.isArray(parsed.devices) && parsed.devices.length) {
          return parsed.devices;
        }
      }
    } catch (e) { /* ignore */ }
    // Optional reuse of FilterWidget localStorage cache shapes if present
    try {
      const fw = localStorage.getItem('filterWidgetV7APICache');
      if (fw) {
        const parsed = JSON.parse(fw);
        // Heuristic: look for a cached /device/devices payload with items
        if (parsed && typeof parsed === 'object') {
          const keys = Object.keys(parsed);
          for (let i = 0; i < keys.length; i++) {
            const entry = parsed[keys[i]];
            const data = entry && (entry.data || entry);
            const items = extractItems(data);
            if (items.length > 50) return items; // partial reuse only if substantial
          }
        }
      }
    } catch (e) { /* ignore */ }
  ...
  /** Fetch all device pages with bounded concurrency (no property filter on API). */
  async function fetchAllDevices(signal) {
    ...
    const firstQ = '/device/devices?size=' + PAGE_SIZE + '&offset=0&fields=' + fields;
    ...
    if (totalPages > 1 && firstItems.length >= PAGE_SIZE) {
      ...
      const pages = await mapPool(offsets, CONCURRENCY, async function (offset) {
        const q = '/device/devices?size=' + PAGE_SIZE + '&offset=' + offset + '&fields=' + fields;
  ...
  /**
   * Resolve device names for metadata filters via client-side property matching.
   * Does NOT build systemProperties:"{json}" API filters (invalid on LM).
   */
  async function resolveDeviceNames(urlFilters, signal) {
    ...
    let devices;
    try {
      devices = await fetchAllDevices(signal);
    } catch (err) {
      if (err && err.name === 'AbortError') throw err;
      // If only display names were needed we already returned; meta requires devices.
      // Fall back: if we have display names only path failed — surface friendly error.
    ...
    const matched = devices.filter(function (d) {
      return deviceMatchesMetaFilters(d, activeMeta);
    });
